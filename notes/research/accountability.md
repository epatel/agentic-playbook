...
- **High** — US copyright position. **Upgraded from medium-high on 19 September 2026 by the
  source-verification pass (`57772ad900e3`): the Part 2 PDF was downloaded and extracted with
  `pdftotext`, and the Office's own eight conclusions are quoted below verbatim.** The law-firm
  intermediary is no longer load-bearing, and reading the primary changed two attributions — see
  *Copyright* below.
...
**Resolved 19 September 2026 against the live file in Linus's tree**, which is what this brief and
the staleness table previously said had to happen before the book printed a trailer syntax. The
source of truth is `Documentation/process/coding-assistants.rst` fetched raw from
`git.kernel.org` — not a rendered docs page, not a mailing-list posting. [1a] Its Attribution
section, verbatim:
...
**The live file matches the kernel.org rendering, not the December 2025 patch posting.** The
`AGENT_NAME:MODEL_VERSION` form — `Assisted-by: Claude:claude-3-opus coccinelle sparse` — appears
in the patch as posted [3] and in secondary write-ups as late as August 2026 [50], and is **not in
the tree**. The literal first token is `LLM`; the model name is not recorded at all. Anything
downstream that prints a model version in a kernel trailer is quoting a draft. **The book may now
print the kernel form.**

Two things in the live file were not in this brief at all, and both are useful:

- It **delegates content rules to a second document**, `Documentation/process/generated-content.rst`
  ("For guidelines on content generated by AI coding assistants see:"). `coding-assistants.rst`
  is the process file; it is not the whole policy. [1a]
- It carries a nine-step **"Procedure for finding and fixing bugs"** that an assistant **MUST**
  follow, and the steps are a remarkably good outside statement of the book's own *Make the agent
  prove it* argument: *"For any bug found that is not trivial, verify that it looks real by
  attempting to create a reproducer to demonstrate it"*; *"Build and verify that the fix works
  either using the reproducer or by re-running a complete analysis; drop any fix that doesn't work
  and try another one"*; and step 8, *"Indicate what could not be done. If the fix could not be
  built or tested, or if no reproducer could be produced, say so explicitly: maintainers currently
  waste too much time analyzing unverified reports and untested fixes."* [1a] The last one is a
  named-project, in-tree statement of the evidence-over-verdicts rule, and nothing else in this
  brief is as quotable on it.
...
on **29 January 2025**. [19][20] **The PDF was retrieved and text-extracted on 19 September 2026 by
the source-verification pass; what follows is the Office's own wording and is safe to quote.**

Its eight conclusions and recommendations, verbatim from the Executive Summary: [19]

```
•  Questions of copyrightability and AI can be resolved pursuant to existing law,
   without the need for legislative change.
•  The use of AI tools to assist rather than stand in for human creativity does not
   affect the availability of copyright protection for the output.
...
•  Based on the functioning of current generally available technology, prompts do
   not alone provide sufficient control.
•  Human authors are entitled to copyright in their works of authorship that are
   perceptible in AI-generated outputs, as well as the creative selection,
   coordination, or arrangement of material in the outputs, or creative
   modifications of the outputs.
•  The case has not been made for additional copyright or sui generis protection
   for AI-generated content.
```

And from section V, the Conclusion: "existing legal doctrines are adequate and appropriate to
resolve questions of copyrightability", with "Prompts alone, however, at this stage are unlikely to
satisfy those requirements". On legislation: "Unless and until future developments raise new
problems, the Office does not recommend a change in the law." [19]

**Two attributions in the earlier draft of this brief were wrong, and reading the primary is what
caught them.** Both are quotations the Office reproduces from *other* parties, not the Office's own
words, and both circulate in law-firm summaries as if the Office said them:

- **"copyright law protects only works of human creation"** is the **US District Court for the
  District of Columbia** in *Thaler*, quoted by the Office in its background section. (Note also
  that the bracketed "[original]" in the earlier draft appears nowhere; the court's sentence has no
  such word.) [19]
- **"selection of a single output is not itself a creative act"** is the **Kernochan Center**, in
  comments submitted in response to the Office's Notice of Inquiry, quoted approvingly by the
  Office. [19]

The phrase **"the selection, coordination, and arrangement of the human-authored and AI-generated
material"** *is* the Office's own, and can be attributed to it directly. [19]
...
| Fedora | Permitted with accountability and disclosure. "The contributor is always the author and is fully accountable for the entirety of these contributions." Disclosure **MUST** happen "when the significant part of the contribution is taken from a tool without changes"; for git, "the recommended method is an `Assisted-by:` commit message trailer". AI **MUST NOT** be "the sole or final arbiter in making a substantive or subjective judgment on a contribution", nor be used to evaluate a person's standing. | **v1.0, 2025-10-24**, verified against the canonical source | [10a] |
...
**1. ~~The kernel's `Assisted-by:` format.~~ SETTLED, 19 September 2026 (`57772ad900e3`).** The
live file in Linus's tree was fetched raw from `git.kernel.org` [1a] and says `Assisted-by: LLM
[TOOL1] [TOOL2]`, example `Assisted-by: LLM coccinelle sparse`. The rendering was not degraded; the
`AGENT_NAME:MODEL_VERSION` form was a **draft that did not survive merge**, and the 2026 write-ups
still quoting it [50] are quoting the patch posting [3], not the kernel. **The book may print the
kernel trailer.** It should not print a model version inside one, and the only residual caveat is
the ordinary one in the staleness table: the kernel edits its process docs freely, so tell the
reader where the file is.
...
- ~~**Fedora's canonical policy page** is behind an Anubis anti-scraping challenge.~~ **Resolved 19
  September 2026 (`57772ad900e3`).** The rendered page is still behind Anubis and now also 404s —
  the URL this brief carried, `.../policy/ai-assisted-contributions/`, is dead. **The canonical text
  is in version control and is not protected at all**: the Council docs moved from Pagure to
  Forgejo, and the policy is `council/modules/ROOT/pages/policy/ai-contribution-policy.adoc` in
  `https://forge.fedoraproject.org/council/docs`, clonable with plain `git clone`. [10a] Read the
  repository, not the site. Four corrections follow from doing so, all small and all the kind that
  survive into print:
  1. The approved document is **`v1.0, 2025-10-24`**, authored "Jason Brooks; Fedora Council; Fedora
     community". This brief said "Council approved 22 Oct 2025", which is LWN's report date [11].
     Git history: added 2025-10-24, with a 2026-01-15 commit adding the version and approval date.
  2. Accountability reads "fully accountable for **the entirety of** these contributions", not
     "fully accountable for their contributions".
  3. The trailer is **not** `Assisted-by: <name of code assistant>` — that placeholder is the
     proposal's [10]. The approved text says "For contributions tracked in git, the recommended
     method is an `Assisted-by:` commit message trailer", with two examples: `Assisted-by: generic
     LLM chatbot` and `Assisted-by: ChatGPTv5`. **Fedora's examples name the product, where the
     kernel's literal token is `LLM`.** Two projects, same trailer key, different values — which
     sharpens rather than weakens the book's "there is no standard" point.
  4. The AI-as-arbiter clause has a half this brief dropped. It is not only about judging *people*:
     "You **MUST NOT** use AI as the sole or final arbiter in making a substantive or subjective
     judgment on a contribution, nor may it be used to evaluate a person's standing within the
     community". It then explicitly carves out objective tooling — "This does not prohibit the use
     of automated tooling for objective technical validation, such as CI/CD pipelines, automated
     testing, or spam filtering" — and closes with "The final accountability for accepting a
     contribution, even if implemented by an automated system, always rests with the human
     contributor who authorizes the action."
...
- ~~**Direct quotations from the Copyright Office Part 2 report.**~~ **Lifted 19 September 2026
  (`57772ad900e3`).** The PDF downloads cleanly over plain HTTPS and extracts with `pdftotext`; only
  `WebFetch` could not read it. The Office's eight conclusions are quoted verbatim above and are
  safe to print. **Two replacement rules, though, because verifying found two misattributions:**
  do not attribute "copyright law protects only works of human creation" to the Office (it is the
  district court in *Thaler*, quoted by the Office), and do not attribute "selection of a single
  output is not itself a creative act" to the Office either (it is the Kernochan Center's comment,
  quoted by the Office). Both are attributed to the Office in the law-firm summaries [21]. If the
  book wants a single sentence in the Office's own voice, use "prompts do not alone provide
  sufficient control."
...
| Kernel trailer is `Assisted-by: LLM …` | Verified in-tree 19 Sep 2026, so the *current* form is not in doubt; but the kernel edits its process docs freely and this one already lost a draft syntax between posting and merge. | Show it as an example of the *shape* of a trailer, and tell the reader to check `Documentation/process/coding-assistants.rst` — in the tree, not on a docs mirror. |
...
[1a] **PRIMARY, and the one to cite** — `Documentation/process/coding-assistants.rst`, raw from the mainline tree — https://git.kernel.org/pub/scm/linux/kernel/git/torvalds/linux.git/plain/Documentation/process/coding-assistants.rst — fetched 19 September 2026 by `57772ad900e3`; 4,063 bytes; confirms `Assisted-by: LLM [TOOL1] [TOOL2]`
[2] AI Coding Assistants — kernel.org latest HTML docs — https://www.kernel.org/doc/html/latest/process/coding-assistants.html — accessed 19 September 2026 — **agrees with [1a]**
...
[10] Council Policy Proposal: Policy on AI-Assisted Contributions — Fedora Community Blog, 25 September 2025 — https://communityblog.fedoraproject.org/council-policy-proposal-policy-on-ai-assisted-contributions/ — accessed 19 September 2026 — **the proposal, which differs from the approved text; superseded by [10a]**
[10a] **PRIMARY, and the one to cite** — *AI-Assisted Contributions Policy*, v1.0, 2025-10-24, Jason Brooks / Fedora Council / Fedora community — `council/modules/ROOT/pages/policy/ai-contribution-policy.adoc` in https://forge.fedoraproject.org/council/docs — cloned 19 September 2026 by `57772ad900e3`. The rendered page at `docs.fedoraproject.org/en-US/council/policy/ai-assisted-contributions/` is behind Anubis **and now 404s**; the repository is open
...
[19] **PRIMARY, and the one to cite** — Copyright and Artificial Intelligence, Part 2: Copyrightability (PDF, 29 January 2025) — https://www.copyright.gov/ai/Copyright-and-Artificial-Intelligence-Part-2-Copyrightability-Report.pdf — **downloaded and extracted 19 September 2026 by `57772ad900e3`** (1.47 MB, 2,243 lines of extracted text). The earlier "not text-extractable" note was a property of `WebFetch`, not of the document
...
[21] Copyrightability of AI Outputs: U.S. Copyright Office Analyzes Human Authorship Requirement — Jones Day, February 2025 — https://www.jonesday.com/en/insights/2025/02/copyrightability-of-ai-outputs-us-copyright-office-analyzes-human-authorship-requirement — accessed 19 September 2026 — **superseded by [19]. Retained only as the trail for the two misattributed quotations described above; do not quote the Office through it**
