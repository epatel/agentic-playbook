...
| [`filter-paired-run/`](filter-paired-run/) | [*Starve the context*](../part-2-plays/context/starve-the-context.md) — a paired run for any token-filtering tool, `rtk` by default, not yet run | `claude`, `rtk`, `git`, `pytest`, an API key, and money |
...
`filter-paired-run/` is the exception to every rule above: it calls a paid API, it keeps its results,
...
`git diff --no-renames --name-only --diff-filter=DM origin/main -- tests/` — the stop gate in *Make the agent prove it*, whose
