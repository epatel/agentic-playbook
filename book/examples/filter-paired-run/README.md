# `filter-paired-run` — does a token filter lower the bill?

A paired run that tests a token-filtering tool against the bill rather than against its own
dashboard. Any tool that installs through Claude Code settings or as an MCP server can go in the
"with" arm. `rtk` is the default, because it is the only tool in the category with published cost
benchmarks to compare against. Unlike every other project in this directory, it **calls a paid API
and spends money**, and it writes its results to a directory you name rather than only to a
temporary one.

```
python3 book/examples/filter-paired-run/run.py check-tasks
python3 book/examples/filter-paired-run/run.py preflight
python3 book/examples/filter-paired-run/run.py run --out book/examples/filter-paired-run/results/<date>
python3 book/examples/filter-paired-run/analyse.py book/examples/filter-paired-run/results/<date>
```
...
|---|---|
| [*Starve the context*](../../part-2-plays/context/starve-the-context.md#worked-example) | "Both tested v0.43.0 in mid-2026; nothing has re-tested it since", of `rtk`, and a results table left blank on purpose |
...
`--filter rtk` is the preset. For anything else, write a JSON file and pass its path. A bare
settings fragment is enough for a hook-based tool. A tool that runs as an MCP server needs the
server too:
...
The server goes in with `--mcp-config`, so `--strict-mcp-config` still keeps every other server out
of both arms. A custom filter has no self-report the runner knows how to read, so `preflight`
refuses it and the leak check in the analysis sees nothing. Check one transcript from each arm by
hand before a paid run, and compare the tool's own savings figure with the measured bill yourself.

## The evidence this adds to
...
the vendor's dated per-million prices. Not the harness's own dollar figure, and not the tool's own
report. Reported as the percentage change with the filter against without, per effort level, with a
95% confidence interval from resampling tasks (10,000 draws).

**Secondary outcomes.** Turns; fresh input, cache reads, cache writes, and output tokens; wall time;
Bash calls; pass rate; cost per passed run; the filter's own tokens-saved figure over the same runs,
where it reports one, set beside the measured change in input tokens; and the number of runs in
which the filter recorded the agent re-fetching output it had cut.
...
- Any "without" run in which the filter acted means the arms leaked into each other.
  Discard that effort level.

## Design

**Tasks.** Twelve bugs seeded into
[`more-itertools`](https://github.com/more-itertools/more-itertools) at tag v11.1.0, commit
`64be96ce`, listed in [`tasks.json`](tasks.json). Each one is a one-line change to library code that
breaks at least one existing test. Each prompt is a user's bug report, checked against what the
seeded bug actually does. The agent has to find the cause, fix it, and get `python3 -m pytest -q
tests` passing without touching `tests/`. The library is two modules of 5,500 and 1,600 lines.
Finding the bug means searching and reading large files, and confirming the fix means reading
test-runner output. Those are the commands a shell-output filter rewrites.
...
settings). The "with" arm adds exactly the filter's settings with `--settings`. For the `rtk`
preset, that is the hook `rtk init --global` installs:
...
**Isolation check.** For `rtk`, `rtk gain -p` reports per working directory, and every run has its
own. So every run records `rtk`'s own claims about that run alone, and a "without" run showing any
...
- **More expensive:** "A paired run on <tool> <version>, <month year>, twelve seeded bugs in a
...
- **No measurable difference:** "A paired run on <tool> <version>, <month year>, could not
...
- **Anything about the agent's knowledge of the filter.** Neither arm's model is told the filter
  exists, which matches how a hook is installed. A setup that also describes the tool in a user
  `CLAUDE.md`, as some do, is a third condition this run does not test. An MCP-based tool is
  different: its tool descriptions tell the model it exists, which is part of what is being tested.
