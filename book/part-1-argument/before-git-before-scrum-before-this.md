...
same file rather than to reconcile their work afterwards. Walter Tichy's 1985 paper introducing RCS
contains a section titled "Locking: A Controversial Issue", and it is unambiguous about the model:
"At most one programmer at a time may lock a particular revision, and only this programmer may check
in the succeeding revision." Breaking somebody's lock sent them an automatic email. Tichy reports
the effect: programmers broke locks "only in real emergencies, or when a co-worker resigns and
leaves locked revisions behind."

A decade later, Microsoft's own Visual SourceSafe glossary records that "in the default work style,
Visual SourceSafe allows only one user at a time to check out a file", and that concurrent editing
was a mode an administrator had to switch on. Subversion shipped in 2000 and did not get automatic
merge tracking until version 1.5 in June 2008; until then, by its own manual's account, a branch
left alone too long could be "near-impossible to merge" back "without a huge number of conflicts".
For most of a decade the dominant tool punished the practice that is now the default.
...
Watch the pull request arrive. GitHub shipped it on 23 February 2008, and the announcement post
describes it, in full, as a way to "tell people who forked from you they need to pull". It is a
notification. GitHub did not itself describe pull requests as code review until "Pull Requests 2.0"
in August 2010, two and a half years later, and even then the practice was thin: researchers
examining 1.9 million pull requests across 2012 and 2013 found 14% of active repositories using pull
requests at all, and 12% of pull requests carrying a review comment.
...
For most of that period the field could not agree on a name for itself. The term in circulation
before Snowbird was "Light", which nobody liked. Alistair Cockburn's objection survives in Jim
Highsmith's account: he did not want "to be referred to as a lightweight attending a lightweight
methodologists meeting", which "sounds like a bunch of skinny, feebleminded lightweight people
trying to remember what day it is". Seventeen people from eight named approaches spent three days in
Utah in February 2001 and produced four value statements. Not a method. A vocabulary.

Adoption was slower than the retelling suggests. In the third quarter of 2010 — fifteen years after
Schwaber's paper — Forrester and *Dr. Dobb's* asked 1,023 IT professionals which methodology most
closely matched their process. Scrum came back at 12.3%, and nearly 29% reported using no formal
...
dozen files and you will usually get back something that compiles and does a fair amount of what you
asked.

What does not exist is everything around that. Consider how few of these have an answer your team
could state out loud in the same words:
...
None of these are hard in the way a hard algorithm is hard. They are unsettled, which is worse,
because a hard question at least has an answer somebody can look up.
...
An analogy that only flatters the present is not worth carrying, so here are three ways this one
does not hold.

The direction of pressure is reversed. Git and Scrum were built by practitioners for their own
problems and spread slowly, by persuasion, from below. The current tools are shipped by vendors on a
release cadence measured in weeks, with a commercial interest in adoption arriving ahead of
practice. Plenty of developers are being told to use them by someone who has not said how, and will
be assessed on the result anyway. That did not happen with Git.

The substrate moves. Git 1.5 did not get better at merging while you slept. Here, the thing your
practice is built on changes underneath the practice. Every convention you adopt encodes an
assumption about something the model cannot currently do reliably, and some of those assumptions
expire without notice — nothing in your repository will tell you which.
...
It was the decade in which the argument happened, which is the only mechanism by which a question of
this kind has ever been settled.
