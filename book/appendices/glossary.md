...
Named failure modes are a separate vocabulary and are indexed at the end of
...
- **Fan-out.** Running several agents on parts of one job at the same time. Distinct from a larger
  ...
  agent the same tokens and compare. See
  [*Decompose into subagents*](../part-2-plays/orchestration/decompose-into-subagents.md).

- **Harness.** The program around the model: the loop that plans, edits, and decides it has
  finished; the tool surface it can call at all; the permission layer governing what runs without
  asking; and the isolation layer deciding what the operating system will refuse regardless. The
  model is the part people have opinions about and the harness is the part that decides what
...
- **Hook.** A piece of your own code the harness runs at a defined point — typically before a tool
  call — which can inspect the call and refuse it. The mechanism to reach for when a rule depends on
...
- **MCP.** The Model Context Protocol: a vendor-neutral protocol by which a server exposes tools
  ...
  descriptions it supplies are context you did not write. See
  [*Wire in the outside world*](../part-2-plays/harness/wire-in-the-outside-world.md).
...
- **Sandbox.** Operating-system isolation — filesystem and network — enforced by the kernel against
  ...
  decision rather than merely shaping a good one, and the only one worth calling a boundary.
...
- **Skill.** A folder holding a `SKILL.md` of metadata plus instructions, alongside any scripts and
  ...
  frontmatter is one vendor's. See
  [*Package repeatable expertise*](../part-2-plays/harness/package-repeatable-expertise.md).
...
- **Token.** The unit a model reads, writes, and is billed in — roughly a short word, though models
  ...
  generations. Both the currency and the capacity constraint.

- **Tool call.** The mechanism by which a model acts: it emits a structured request to run something
  — read a file, execute a command, query a server — the harness runs it, and the result comes back
  into the context window as text. Everything an agent does that is not producing prose is a tool
  call, and every result it receives is something it must then be trusted to have read correctly.
